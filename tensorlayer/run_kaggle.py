# run this scipt if the train successed while prediction failed.
...
train_op_conv = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.9, beta2=0.999,
                                       epsilon=1e-08, use_locking=False).minimize(cost_conv, var_list=network_conv.all_params)
y_conv_op = tf.argmax(tf.nn.softmax(y_conv), 1)

# train
tl.layers.initialize_global_variables(sess)
tl.files.load_and_assign_npz(sess=sess, name='model_conv.npz', network=network_conv)
tl.files.load_and_assign_npz(sess=sess, name='model_dense.npz', network=network_dense)
